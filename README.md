...
* If using an AMD CPU with 3D V-Cache technology, benchmark results may show little to no difference when overclocking or tuning RAM. It is suspected that this happens because the scenario doesn't generate enough highly performance-sensitive data to where some must remain in RAM.
